...
public class BaseDeFait implements List<Fait>, JSONAware, Iterable<Fait> {
    ...
    public boolean add(Fait p) {
        for (int i = 0; i < m_content.size(); ++i) {
            Fait f = m_content.get(i);
            if (f.getVal().equals(p.getVal()) && f.getVar().equals(p.getVar()) && f.egalite() == p.egalite()) {
                if (f.getProba() < p.getProba()) {
                    m_content.set(i, p);
                    return true;
                } else {
                    return false;
